...
# JV-Dataに関するメモ
## データ種別とレコード種別などの関係
- 「情報」と「データ種別」と「レコード種別」の３種類がある
- 情報
  - 「競走馬マスタ」「レース詳細」「速報オッズ（馬連）」「時系列オッズ（馬連）」など
- データ種別
  - 情報をグルーピングしたもの
  - JV-Linkからは、この単位でしか取得できない
  - 例えば、「データ種別＝0B30速報オッズ(全賭式)」に「オッズ2（馬連）」があるが、「データ種別＝0B32速報オッズ(馬連)」にも「オッズ2（馬連）」はある
    - この２例の値は、まったく同一かもしれない（推測）
    - データ種別は、便利機能と考えて設けているのではないか（推測）
      - この結果、すごくわかりづらくなっている🤬
- レコード種別
  - 情報の、１レコードの固定長の"型"
  - 例えば、「RA」は、蓄積系でも速報系でも、複数の情報で利用している
  - もしかしたら、同じレコード種別は、混ざっても、どこかの項目で情報を判別できるかもしれない（推測）
